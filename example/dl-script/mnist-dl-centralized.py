...
job_id = str(os.environ["tfjob_id"]) if "tfjob_id" in os.environ else "UNKNOWN"
link = str(os.environ["webhook_link"]) if "webhook_link" in os.environ else "UNKNOWN"

# Parameters
learning_rate = 0.001
training_iters = int(os.environ["global_steps"]) if "global_steps" in os.environ else 10000
batch_size = 16
display_step = 10
...
with tf.Session() as sess:
    ...
    message = "Time elapsed: {} s, test accuracy: {} %".format(time2 - time1, acc * 100)
    payload = str({
                "sender": job_id,
                "message": message
            })
